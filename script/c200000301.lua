...
function s.penchk(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckEvent(e2)
end

function s.bewdadmchk(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckLocation(tp,LOCATION_GRAVE,1)+c.IsCode(CARD_BLUEEYES_W_DRAGON) or Duel.CheckLocation(tp,LOCATION_GRAVE,1)+c.IsSetCard(0x140)+c.IsType(TYPE_TUNER) or Duel.CheckLocation(tp,LOCATION_DECK,1)+c.IsCode(CARD_BLUEEYES_W_DRAGON) or Duel.CheckLocation(tp,LOCATION_DECK,1)+c.IsSetCard(0x140)+c.IsType(TYPE_TUNER)
end
...
function s.spsmop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.MoveToField(c,tp,tp,LOCATION_MZONE,POS_FACEUP,true) then
	Duel.SynchroSummon()
end
