...
function s.filter(c)
	return c:IsFaceup() and c:IsMonster() and c:IsLevelAbove(5)
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()	
	local g=Duel.GetMatchingGroup(aux.FaceupFilter(s.filter,e),tp,LOCATION_MZONE,LOCATION_MZONE,nil)
	local att=Duel.AnnounceAnotherAttribute(g,tp)
	local sc=g:FilterSelect(tp,Card.IsAttributeExcept,1,1,nil,att):GetFirst()
	if sc then
