...
s.listed_series={0x7981}
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(1)
	return true
end
function s.filter1(c,e,tp,ft)
	return c:IsType(TYPE_RITUAL) and (ft>0 or (c:GetSequence()<5 and c:IsControler(tp))) 
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_DECK,0,1,nil,c:GetCode(),e,tp)
end
function s.filter2(c,tcode,e,tp)
	return c:IsSetCard(0x7981) and c.beyonder_mode and c.beyonder_mode==tcode and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
