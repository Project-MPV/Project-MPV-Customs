...
function s.filter1(c,e,tp,ft)
	return c:IsType(TYPE_XYZ) and (ft>0 or (c:GetSequence()<5 and c:IsControler(tp))) 
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_DECK,0,1,nil,c:GetCode(),e,tp)
end
function s.filter2(c,tcode,e,tp)
	return c:IsSetCard(0x7982) and c.eclipse_mode and c.eclipse_mode==tcode and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
