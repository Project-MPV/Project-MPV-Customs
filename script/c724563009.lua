...
function s.nefafilter(c)
	return c:IsSetCard(0xfc12) and c:IsAbleToHand()
end
function s.rescon(e,tp,eg,ep,ev,re,r,rp)
    return Duel.GetFlagEffect(tp,id)==0 
        and Duel.IsExistingMatchingCard(s.nefafilter,tp,LOCATION_DECK,0,1,nil)
end
function s.rtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.nefafilter,tp,LOCATION_DECK,0,1,nil) end
